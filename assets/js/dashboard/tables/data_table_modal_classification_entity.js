...
export function createTable(page, options){
    ...
    document.getElementById("data-modal-table-csv").href = `/api/data-per-classification-and-entity/${window.project_id}/?format=csv&&classification=${options.classification}&entity=${options.entity}&` + new URLSearchParams({
    ...
    fetch(`/api/data-per-classification-and-entity/${window.project_id}/?page=${page}&page-size=${pageSize}&classification=${options.classification}&entity=${options.entity}&` + urlParams)
