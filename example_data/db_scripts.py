...
def add_data(project, source, text, lang, with_entities=False, aspect_model=None,aspect=None):
    ...
    sentiment = requests.post('http://api.repustate.com/v4/{APIKEY}/score.json'.format(APIKEY = APIKEY), {'text': text, 'lang': lang}).json()['score']

    entities = None
    if with_entities:
        entities = requests.post('http://api.repustate.com/v4/{APIKEY}/entities.json'.format(APIKEY = APIKEY), {'text': text, 'lang': lang}).json()
    
    aspects = None
    if aspect_model is not None:
        aspects = requests.post('http://api.repustate.com/v4/{APIKEY}/aspect.json'.format(APIKEY = APIKEY), {'text': text, 'lang': lang, 'model': aspect_model}).json()
    else:
